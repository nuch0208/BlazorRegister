...
namespace JWTAuthAPI.Services
{
    public class UserService : IUserService
    {
        ...
        public UserService(MyWorldDbContext worldDbContext, IOptions<TokenSettings> tokenSettings)
        {
        ...
        {
            var userAlreadyExist = _worlDbContext.User.Any(_ => _.Email.ToLower() == email.ToLower());
        ...
        public async Task<(bool IsLoginSuccess, JWTTokenResponseDto TokeResponse)> LoginAsync(LoginDto loginPayload)
        {
            ...
            var user = await _worlDbContext.User.Where(_ => _.Email.ToLower() == loginPayload.Email.ToLower()).FirstOrDefaultAsync();
